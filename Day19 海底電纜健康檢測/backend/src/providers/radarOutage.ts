...
export function buildRadarHeaders(){
  ...
  if(CONFIG.CLOUDFLARE_API_TOKEN) headers['Authorization']=`Bearer ${CONFIG.CLOUDFLARE_API_TOKEN}`;
  return headers;
}

export async function fetchRadarOutages(opts:{location?:string;asn?:string;dateRange?:string;dateStart?:string;dateEnd?:string;limit?:number;offset?:number}){
  const url=new URL(RADAR_OUTAGES);
  if(opts.location)url.searchParams.set('location',opts.location);
  if(opts.asn)url.searchParams.set('asn',opts.asn);
  if(opts.dateRange)url.searchParams.set('dateRange',opts.dateRange);
  if(opts.dateStart)url.searchParams.set('dateStart',opts.dateStart);
  if(opts.dateEnd)url.searchParams.set('dateEnd',opts.dateEnd);
  url.searchParams.set('limit',String(opts.limit??100)); url.searchParams.set('offset',String(opts.offset??0));
...
let _authCache:{ok:boolean; ts:number; status?:number; message?:string} | null = null;
export async function checkRadarToken(){
  const now = Date.now();
  if (_authCache && (now - _authCache.ts) < 10*60*1000) return _authCache;
  ...
  try{
    ...
    if(!ok){
      try{ const j = await r.json(); msg = JSON.stringify(j); }catch{ msg = await r.text().catch(()=>undefined); }
    }
...
export function normalizeAnnotations(obj:any): any[] {
  try{
    if(!obj) return [];
    const arr = obj?.result?.annotations ?? obj?.result?.data ?? obj?.annotations ?? obj?.data ?? [];
    if (Array.isArray(arr)) return arr;
    if (Array.isArray(obj?.result?.annotations?.data)) return obj.result.annotations.data;
    return [];
  }catch{ return []; }
}