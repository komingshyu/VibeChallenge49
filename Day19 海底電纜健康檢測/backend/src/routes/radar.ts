...
r.get('/outages', async (q,s)=>{ 
  try{ 
    const { location='TW',asn='',dateRange='30d'}=q.query as any; 
    s.json(await fetchRadarOutages({location,asn,dateRange,limit:100,offset:0})); 
  }catch(e:any){ s.status(500).json({error:e.message||'radar error'});} 
});

// 統計摘要：每日事件數與總數（前端更好消化）
r.get('/outages/summary', async (q,s)=>{
  try{
    const { location='TW', asn='', dateRange='90d' } = q.query as any;
