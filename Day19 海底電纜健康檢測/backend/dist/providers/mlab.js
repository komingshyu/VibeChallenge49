...
async function getBigQuery() { try {
    ...
    throw new Error('BigQuery client not installed. Run scripts\\enable-mlab.bat.');
} }
export async function queryMlabSummary(params) {
    const bq = await getBigQuery();
    const days = params.days ?? 7;
    const country = params.country ?? 'TW';
    const sql = `
WITH base AS (
...
FROM base b JOIN up u USING (hour_ts) GROUP BY b.hour_ts ORDER BY b.hour_ts ASC`; // @ts-ignore
    const [rows] = await bq.query({ query: sql, params: { country, days } });
    return rows;
}
